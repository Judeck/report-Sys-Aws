...
//   @DeleteMapping
     ...
     public void delteFile(@PathVariable String reqId, @PathVariable FileType type, HttpServletResponse response) throws IOException {
         log.debug("Got Request to Download File - type: {}, reqid: {}", type, reqId);
         InputStream fis = reportService.getFileBodyByReqId(reqId, type);
         reportService.deleteReportById(reqId);
         log.debug("delte File:{}", reqId);
     }


    /*@DeleteMapping("/excel/{id}")
    public ResponseEntity<GeneralResponse> deleteExcel(@PathVariable String id) throws FileNotFoundException {
        log.debug("Got Request to Delete File:{}", id);
        var response = new GeneralResponse();
        ExcelFile fileDeleted = reportService.deleteFile(id);
